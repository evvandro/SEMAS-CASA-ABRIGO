...
type SectorCapacityInput = Pick<Sector, 'active' | 'capacity' | 'occupied' | 'blockedBeds'>

export function normalizeBlockedBeds(beds?: string[] | null) {
  return Array.from(new Set((beds ?? []).map((bed) => normalizeBedKey(bed)).filter(Boolean)))
}
...
export function parseBedNumber(value: string | null | undefined, capacity: number) {
...
export function getBedKeyFromValue(value: string | null | undefined, capacity: number) {
  if (!value) return ''
...
export function buildBedOptions({
  ...
  const blocked = new Set(
    normalizeBlockedBeds(blockedBeds).filter((blockedBed) => isBedWithinCapacity(blockedBed, normalizedCapacity)),
  )
  ...
  if (currentValue && currentKey && !options.some((option) => getBedKeyFromValue(option.value, normalizedCapacity) === currentKey)) {
...
export function getSectorCapacitySummary(sector: SectorCapacityInput) {
  ...
  const usableCapacity = sector.active ? Math.max(capacity - blockedBedsCount, 0) : 0
  const freeBeds = sector.active ? Math.max(usableCapacity - occupied, 0) : 0
  const occupancyPercent = usableCapacity > 0
