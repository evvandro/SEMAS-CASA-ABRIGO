...
export function CadastroDrawer({
  ...
  const set = <K extends keyof CadastroPayload>(
    k: K,
    v: CadastroPayload[K],
  ) => {
    setForm((f) => ({ ...f, [k]: v }));
  ...
  const bedOptions = useMemo(() => {
    ...
    return buildBedOptions({
      ...
      occupiedBeds: rows
        .filter(
          (row) =>
            row.sectorId === selectedSector.id &&
            row.apiId !== initialRow?.apiId,
        )
        .map((row) => row.bed),
      currentBed: form.bed,
    });
        ...
        >
          <Box>
            ...
            <Typography variant="caption">
              {mode === 'edit'
                ? 'Atualização dos dados essenciais'
        ...
        <Box sx={{ flex: 1, overflowY: 'auto', p: 3 }}>
          ...
          >
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  onChange={(e) => set('gestante', e.target.checked)}
                />
              }
              label="Gestante"
            />
            <FormControlLabel
              control={
                <Checkbox
                  checked={form.cronica}
                  onChange={(e) => set('cronica', e.target.checked)}
                />
              }
            ...
            <FormControlLabel
              control={
                <Checkbox
                  checked={form.idoso}
                  onChange={(e) => set('idoso', e.target.checked)}
                />
              }
              label="Idoso 60+"
            />
          ...
          >
            {sectors.map((s) => {
              ...
              const noUsefulBeds =
                s.active &&
                summary.capacity > 0 &&
                summary.usableCapacity === 0;
              const full =
                s.active &&
                summary.usableCapacity > 0 &&
                summary.occupied >= summary.usableCapacity;
              const unavailable =
                !selected && (interdicted || noUsefulBeds || full);
          ...
          <Box sx={{ mt: 2 }}>
            <FormControl
              fullWidth
              disabled={!selectedSector || !selectedSector.active}
            >
              ...
              <FormHelperText>
                {selectedSector
                  ? 'Somente leitos livres aparecem na lista.'
