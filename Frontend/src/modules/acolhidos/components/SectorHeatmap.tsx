...
import { bedKey, getSectorCapacitySummary, parseBedNumber } from '../utils/sectorCapacity'
...
const cellBg = (active: boolean, pct: number, usableCapacity: number, occupied: number) => {
...
const bedStateMeta = (state: BedState, sectorColor: string) => {
  if (state === 'priority') return { tip: 'Prioritário', bg: '#DC2626', border: 'none' }
  if (state === 'occupied') return { tip: 'Ocupado', bg: sectorColor, border: 'none' }
  if (state === 'blocked') return { tip: 'Interditado', bg: '#FEE2E2', border: '1px solid #FCA5A5' }
...
const buildBeds = (
  ...
  const beds = Array.from({ length: sector.capacity }, (_, index) => {
    ...
    return {
      number,
      state: (!sector.active || sector.blockedBedKeys.has(bedKey(number)))
        ? 'blocked' as BedState
        : 'free' as BedState,
    }
  })
  const pending: Acolhido[] = []
  const orderedOccupants = [...occupants].sort((a, b) => Number(b.alerts.length > 0) - Number(a.alerts.length > 0))
  ...
  pending.forEach((occupant) => {
    const target = beds.find((bed) => bed.state === 'free')
      ?? beds.find((bed) => bed.state === 'blocked')

    if (target) target.state = occupant.alerts.length > 0 ? 'priority' : 'occupied'
  })
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2, flexWrap: 'wrap', gap: 1 }}>
        <Box>
          ...
          <Typography variant="caption">
            ...
            {totalBlockedBeds > 0 ? ` · ${blockedBedsLabel(totalBlockedBeds)}` : ''} · clique para filtrar
          </Typography>
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
          ...
          <LegendItem label="Interditado" color="#FEE2E2" border="1px solid #FCA5A5" />
        </Box>
      </Box>
          ...
          return (
            <Paper
              ...
              sx={{
                p: 1.5,
                cursor: 'pointer',
                bgcolor: cellBg(s.active, s.occupancyPercent, s.usableCapacity, s.occupied),
                borderColor: isActive ? 'primary.main' : !s.active || s.blockedBedsCount > 0 ? '#FCA5A5' : 'divider',
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                ...
                <Typography sx={{ fontSize: 13, fontWeight: 600, minWidth: 0 }} noWrap>{s.name}</Typography>
                <Typography
                  variant="caption"
                  color={!s.active || s.blockedBedsCount > 0 ? 'error.main' : 'text.secondary'}
                  sx={{ ml: 'auto', flexShrink: 0 }}
                >
                  {!s.active ? 'Interditado' : s.blockedBedsCount > 0 ? blockedBedsLabel(s.blockedBedsCount) : s.sub}
                </Typography>
              </Box>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography variant="caption">
                  <strong style={{ color: '#0F172A' }}>{s.occupied}</strong> / {s.usableCapacity} vagas úteis
                </Typography>
