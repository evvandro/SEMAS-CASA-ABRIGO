...
export function AcolhidosPage() {
  ...
  const handleAcolhidoAction = async (
    action: AcolhidoAction,
    row: Acolhido,
  ) => {
    if (action === 'view') {
      state.openFicha(row);
    ...
    if (action === 'print') {
      try {
        ...
        await openAcolhidoFichaPdf(
          detail,
          state.sectorMap[detail.sectorId],
          user?.name,
        );
        state.setToast({
          message: 'PDF da ficha gerado com sucesso.',
          severity: 'success',
        });
      } catch {
        state.setToast({
          message: 'Não foi possível gerar o PDF da ficha.',
          severity: 'error',
        });
      }
      return;
  ...
  const handleGenerateLabel = async ({
    ...
    await openPertencesLabelPdf(
      updated,
      state.sectorMap[updated.sectorId],
      shelterName,
      belongings,
    );
    state.setLabelRow(null);
    state.setToast({
      message: 'Etiqueta de pertences gerada com sucesso.',
      severity: 'success',
    });
  };
  ...
  return (
    <Box>
      ...
      <PertencesLabelDialog
        row={state.labelRow}
        sector={
          state.labelRow ? state.sectorMap[state.labelRow.sectorId] : undefined
        }
