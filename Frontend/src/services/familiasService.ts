...
export function toFamilia(apiFamilia: ApiFamilia): Familia {
  return {
    ...
    acolhidosCount:
      apiFamilia.acolhidos_count ??
      apiFamilia.acolhidos?.filter((membro) => membro.ativo).length ??
      0,
    ...
    membros: apiFamilia.acolhidos?.map((membro) => ({
      ...toAcolhido(membro),
      active: membro.ativo,
    })),
  };
...
export async function fetchFamilias(params?: {
  search?: string;
  setorId?: string;
  status?: 'ativos' | 'saida';
}): Promise<Familia[]> {
...
export async function createFamilia(
  payload: FamiliaRequestPayload,
): Promise<Familia> {
  const res = await api.post<{ data: ApiFamilia }>('/familias', payload);
  return toFamilia(res.data.data);
}

export async function registerFamiliaSaida(
  id: number,
  ...
  const res = await api.post<{ data: ApiFamilia }>(
    `/familias/${id}/saida`,
    toSaidaRequestPayload(isoDate, tipoSaida, payload),
  );
