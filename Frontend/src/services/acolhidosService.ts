...
export function toAcolhido(api: ApiAcolhido): Acolhido {
  const alerts: AlertCategory[] = (
    ['pcd', 'gestante', 'cronica', 'idoso'] as AlertCategory[]
  ).filter((k) => api[k]);
  return {
    apiId: api.id,
    ...
    entryTime:
      normalizeTime(api.hora_entrada) ??
      normalizeTime(getObservationValue(api.observacoes, 'Hora da entrada')),
...
export async function createAcolhido(
  payload: CadastroPayload,
): Promise<Acolhido> {
  return createAcolhidoRecord(toCadastroPayload(payload));
}

export async function createAcolhidoRecord(
  payload: AcolhidoRequestPayload,
): Promise<Acolhido> {
...
export async function updateAcolhidoRecord(
  ...
  const res = await api.patch<{ data: ApiAcolhido }>(
    `/acolhidos/${apiId}`,
    payload,
  );
  return toAcolhido(res.data.data);
...
export async function registerAcolhidoSaida(
  apiId: number,
  dataSaida: string,
  tipoSaida: string,
  detalhesSaida: SaidaPayload,
): Promise<Acolhido> {
  const res = await api.post<{ data: ApiAcolhido }>(
    `/acolhidos/${apiId}/saida`,
    toSaidaRequestPayload(dataSaida, tipoSaida, detalhesSaida),
  );
  return toAcolhido(res.data.data);
}

export function toSaidaRequestPayload(
  dataSaida: string,
  tipoSaida: string,
  detalhesSaida: SaidaPayload,
) {
  const encaminhamentos =
    detalhesSaida.encaminhamentos.includes('Outro servico') &&
    detalhesSaida.encaminhamentoOutro
