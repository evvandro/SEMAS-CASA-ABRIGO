...
export async function createRecebimento(payload: RecebimentoPayload): Promise<Recebimento> {
  const res = await api.post<{ data: Recebimento }>('/recebimentos-materiais', payload)
  return res.data.data
}

export async function createEntregaLote(payload: EntregaLotePayload): Promise<Entrega[]> {
