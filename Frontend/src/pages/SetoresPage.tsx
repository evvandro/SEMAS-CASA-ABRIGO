...
const normalizeBlockedBeds = (beds?: string[] | null) =>
  Array.from(
    new Set((beds ?? []).map((bed) => String(bed).trim()).filter(Boolean)),
  );
...
const buildSectorDetail = (
  setor: ApiSetor,
  allAcolhidos: Acolhido[],
): SectorDetail => {
  ...
  const manuallyBlocked = new Set(
    normalizeBlockedBeds(setor.leitos_interditados),
  );
  ...
  const percent =
    capacity > 0 ? Math.min(Math.round((occupied / capacity) * 100), 100) : 0;
  const blockedBeds = setor.ativo
    ? beds.filter((bed) => bed.blocked && !bed.occupant).length
    : Math.max(capacity - occupied, 0);
  const freeBeds = setor.ativo
    ? Math.max(capacity - occupied - blockedBeds, 0)
    : 0;
...
export function SetoresPage() {
  ...
  const totalCapacidade = details.reduce(
    (acc, detail) => acc + detail.capacity,
    0,
  );
  const totalOcupado = details.reduce(
    (acc, detail) => acc + detail.occupied,
    0,
  );
  const totalLivres = details.reduce((acc, detail) => acc + detail.freeBeds, 0);
  const setoresInterditados = details.filter(
    (detail) => !detail.setor.ativo,
  ).length;
  const totalLeitosInterditados = details.reduce(
    (acc, detail) => acc + detail.blockedBeds,
    0,
  );
  ...
  const syncSetor = (updated: ApiSetor) => {
    setSetores((prev) =>
      prev.map((setor) => (setor.id === updated.id ? updated : setor)),
    );
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSetores((prev) =>
        prev.filter((setor) => setor.id !== confirmDelete.id),
      );
  ...
  const handleToggleBed = async (setor: ApiSetor, bedNumber: number) => {
    ...
    setUpdatingStatus(true);
    try {
      const updated = await updateSetor(setor.id, {
        leitos_interditados: next,
      });
      syncSetor(updated);
      setToast(
        next.includes(key)
          ? `${bedLabel(bedNumber)} interditado.`
    ...
    <Stack spacing={3}>
      ...
      <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
        ...
        >
          <Box>
            ...
            <Typography variant="caption">
              {totalOcupado} / {totalCapacidade} leitos ocupados · {totalLivres}{' '}
              livres
              {setoresInterditados + totalLeitosInterditados > 0
                ? ` · ${setoresInterditados + totalLeitosInterditados} interdições`
                : ''}{' '}
              · clique para abrir
            </Typography>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box
              sx={{
                ...
                borderRadius: 0.5,
                background:
                  'linear-gradient(to right, #DCFCE7 0%, #FEF3C7 50%, #FEE2E2 80%, #FCA5A5 100%)',
                border: '1px solid',
                borderColor: 'divider',
              }}
            />
        ...
        {setores.length === 0 ? (
          ...
          >
            {details.map((detail) => {
              const { setor, beds, capacity, occupied, percent, status } =
                detail;
              const meta = statusMeta[status];
                  ...
                  >
                    <Box
                      sx={{
                        ...
                        height: 10,
                        borderRadius: 0.5,
                        bgcolor: status === 'blocked' ? meta.color : setor.cor,
                        flexShrink: 0,
                      }}
                    />
                      ...
                      >
                        ...
                        <Tooltip title="Remover">
                          <IconButton
                            size="small"
                            color="error"
                            onClick={() => setConfirmDelete(setor)}
                          >
      ...
      <Drawer
        ...
        slotProps={{
          paper: { sx: { width: { xs: '100vw', sm: 680, md: 760 } } },
        }}
      >
        {selectedDetail && (
          <Box
            ...
            >
              ...
              >
                <Box>
                  ...
                  >
                    <Box
                      sx={{
                        ...
                        height: 12,
                        borderRadius: 0.75,
                        bgcolor: selectedDetail.setor.ativo
                          ? selectedDetail.setor.cor
                          : statusMeta.blocked.color,
                      }}
                    />
                  ...
                  <Typography color="text.secondary">
                    {selectedDetail.occupied} acolhido ativo |{' '}
                    {selectedDetail.freeBeds} leitos livres
                    {selectedDetail.blockedBeds > 0
                      ? ` | ${selectedDetail.blockedBeds} leitos interditados`
                      : ''}
                  </Typography>
                </Box>
              ...
              {isAdmin && (
                <Paper
                  ...
                  sx={{
                    ...
                    mb: 2,
                    borderRadius: 1,
                    borderColor: selectedDetail.setor.ativo
                      ? 'divider'
                      : statusMeta.blocked.border,
                    bgcolor: selectedDetail.setor.ativo
                      ? 'background.paper'
                      : statusMeta.blocked.bg,
                  }}
                >
                  <Stack
                    direction={{ xs: 'column', sm: 'row' }}
                  ...
                  >
                    <Box>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {selectedDetail.setor.ativo
                          ? 'Setor operacional para novas alocações.'
                          : 'Setor interditado para novas alocações.'}
                      </Typography>
                    </Box>
              ...
              <Stack spacing={2}>
                <Box>
                  ...
                  {selectedDetail.occupants.length > 0 ? (
                    ...
                    <Paper
                      variant="outlined"
                      sx={{ p: 2, borderRadius: 1, color: 'text.secondary' }}
                    >
                ...
                <Box>
                  ...
                  {selectedDetail.beds.length > 0 ? (
                    <>
                      <Paper
                        variant="outlined"
                        sx={{ p: 1.5, borderRadius: 1, mb: 1.5 }}
                      >
                        <Box
                          sx={{
                            display: 'grid',
                        ...
                        >
                          {selectedDetail.beds.map((bed) => (
                            <BedCell
                              key={bed.number}
                              bed={bed}
                              color={selectedDetail.setor.cor}
                              compact
                            />
                          ))}
                        </Box>
                      ...
                      <Grid container spacing={1}>
                        {selectedDetail.beds.map((bed) => {
                          const individuallyBlocked = normalizeBlockedBeds(
                            selectedDetail.setor.leitos_interditados,
                          ).includes(bedKey(bed.number));
                          const canToggleBed =
                            isAdmin &&
                            selectedDetail.setor.ativo &&
                            !bed.occupant;
                          const bedStatus = bed.occupant
                            ? 'Ocupado'
                            : bed.blocked
                              ? 'Interditado'
                              : 'Livre';
        ...
        <DialogContent>
          <Stack spacing={2.5} sx={{ pt: 1 }}>
            ...
            <TextField
              ...
              onChange={(event) =>
                setForm((prev) => ({ ...prev, capacidade: event.target.value }))
              }
              fullWidth
            ...
            <Box>
                ...
                {PRESET_COLORS.map((color) => (
                  <Box
                    ...
                    sx={{
                      ...
                      outlineColor:
                        form.cor === color ? 'text.primary' : 'transparent',
                      outlineOffset: 2,
              ...
              <TextField
                ...
                value={form.cor}
                onChange={(event) =>
                  setForm((prev) => ({ ...prev, cor: event.target.value }))
                }
                size="small"
                fullWidth
                slotProps={{
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Deseja remover o setor <strong>{confirmDelete?.nome}</strong>? Não é
