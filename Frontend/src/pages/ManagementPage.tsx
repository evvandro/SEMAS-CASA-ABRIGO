...
export function ManagementPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (active)
          setError('Não foi possível carregar os indicadores de gestão.');
      } finally {
  ...
  const totalCapacity = sectors.reduce(
    (acc, sector) => acc + sector.capacity,
    0,
  );
  const occupiedCapacity = sectors.reduce(
    (acc, sector) => acc + sector.occupied,
    0,
  );
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <StatCard
            ...
            helper={
              totalCapacity > 0
                ? `${occupiedCapacity} de ${totalCapacity} leitos`
                ...
                <Stack spacing={2}>
                  {busiestSectors.map((sector) => {
                    const percent =
                      sector.capacity > 0
                        ? Math.min(
                            (sector.occupied / sector.capacity) * 100,
                            100,
                          )
                        : 0;
                    ...
                    return (
                      <Box key={sector.id}>
                        ...
                        >
                          ...
                          >
                            {sector.capacity > 0
                              ? `${sector.occupied}/${sector.capacity}`
                              : `${sector.occupied} pessoas`}
                          </Typography>
                        </Stack>
                        <LinearProgress
                          variant="determinate"
          ...
          >
            <Stack spacing={2}>
              ...
              <Grid container spacing={1.5}>
                {(Object.keys(alertLabels) as AlertCategory[]).map(
                  (category) => (
              ...
              {recentRows.length > 0 ? (
                recentRows.map((row) => (
                  <TableRow key={row.id} hover>
                    ...
                    <TableCell>
                      {sectorMap[row.sectorId]?.name ?? '-'}
                    </TableCell>
                    <TableCell>
                      {formatEntryDateTime(row.entry, row.entryTime)}
                    </TableCell>
                    <TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={5} sx={{ py: 6, textAlign: 'center' }}>
                    <Typography color="text.secondary">
                      Nenhum acolhido ativo encontrado.
                    </Typography>
                  </TableCell>
                </TableRow>
              )}
