...
function getApiErrorMessage(error: unknown): string {
  ...
  return firstValidationMessage ?? response?.data?.message ?? 'Falha ao registrar saida. Verifique os dados e tente novamente.'
}
...
export function SaidasPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast.error('Nao foi possivel carregar pessoas e familias ativas.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const openDialog = async () => {
    if (tab === 'pessoas' && !selectedAcolhido) {
      toast.error('Selecione uma pessoa antes de abrir a ficha de saida.')
      return
    }

    if (tab === 'familias') {
      if (!selectedFamilia) {
        toast.error('Selecione uma familia antes de abrir a ficha de saida.')
        return
      }
  ...
  const handleSaveSaida = async (payload: SaidaPayload) => {
    ...
    try {
      if (tab === 'familias' && selectedFamilia) {
        const familiaSaida = await registerFamiliaSaida(selectedFamilia.id, payload)
        setFamilias(prev => prev.filter(familia => familia.id !== selectedFamilia.id))
        setAcolhidos(prev => prev.filter(acolhido => acolhido.familyId !== selectedFamilia.id))
        setHistoricoFamilias(prev => [familiaSaida, ...prev])
        const membrosSaidos = selectedFamilia.membros?.map(membro => ({
          ...membro,
  ...
  return (
    <Box>
      ...
      <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider' }}>
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ p: 3 }} alignItems={{ xs: 'stretch', md: 'center' }}>
          {tab === 'pessoas' ? (
            <Autocomplete
              options={acolhidos}
              getOptionLabel={(option) => `${option.name} | ${option.cpf || 'CPF nao informado'}${option.familyCode ? ` | ${option.familyCode}` : ''}`}
              value={selectedAcolhido}
              onChange={(_, newValue) => setSelectedAcolhido(newValue)}
              loading={loading}
              sx={{ minWidth: { md: 480 }, flex: 1 }}
              renderInput={(params) => <TextField {...params} label="Pessoa acolhida ativa" />}
            />
          ) : (
            <Autocomplete
              options={familias}
              getOptionLabel={(option) => `${option.codigo} | ${option.responsavelNome ?? 'Responsavel nao informado'} | ${option.acolhidosCount} membro(s)`}
              value={selectedFamilia}
              onChange={(_, newValue) => setSelectedFamilia(newValue)}
              loading={loading}
              sx={{ minWidth: { md: 480 }, flex: 1 }}
              renderInput={(params) => <TextField {...params} label="Familia ativa" />}
            />
          )}

          <Button variant="contained" startIcon={<LogoutIcon />} onClick={openDialog}>
            Abrir ficha de saida
          </Button>
        </Stack>
      </Paper>

      <Paper elevation={0} sx={{ mt: 3, border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
            ...
            <TableBody>
              {historicoAcolhidos.map((acolhido) => (
                <TableRow key={acolhido.apiId} hover>
                  <TableCell>{formatDateTime(acolhido.exitDate, acolhido.exitTime)}</TableCell>
                  <TableCell>
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>{acolhido.name}</Typography>
                    <Typography variant="caption" color="text.secondary">{acolhido.id}</Typography>
                  </TableCell>
                  <TableCell>{acolhido.familyCode ? <Chip size="small" label={acolhido.familyCode} /> : 'Pessoa sozinha'}</TableCell>
                  <TableCell>{valueOrFallback(acolhido.exitType)}</TableCell>
                  <TableCell>{valueOrFallback([acolhido.exitDestination, acolhido.exitCity].filter(Boolean).join(' - '))}</TableCell>
                  <TableCell>{valueOrFallback(acolhido.exitResponsible)}</TableCell>
                </TableRow>
              ))}
              {historicoAcolhidos.length === 0 ? <EmptyHistoryRow colSpan={6} /> : null}
            </TableBody>
          </Table>
        ) : (
          <Table size="small">
            ...
            <TableBody>
              {historicoFamilias.map((familia) => (
                <TableRow key={familia.id} hover>
                  <TableCell>{formatDateTime(familia.dataSaida, familia.horaSaida)}</TableCell>
                  <TableCell>
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>{familia.codigo}</Typography>
                    <Typography variant="caption" color="text.secondary">{familia.responsavelNome ?? 'Responsavel nao informado'}</Typography>
                  </TableCell>
                  <TableCell>{familia.acolhidosCount}</TableCell>
                  <TableCell>{valueOrFallback(familia.tipoSaida)}</TableCell>
                  <TableCell>{valueOrFallback([familia.destinoInformado, familia.municipioDestino].filter(Boolean).join(' - '))}</TableCell>
                  <TableCell>{valueOrFallback(familia.responsavelDesligamento)}</TableCell>
                </TableRow>
              ))}
              {historicoFamilias.length === 0 ? <EmptyHistoryRow colSpan={6} /> : null}
            </TableBody>
          </Table>
        )}
      </Paper>
