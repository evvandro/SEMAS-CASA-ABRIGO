...
export function EstoquePage() {
  ...
  const [destinoTipo, setDestinoTipo] =
    useState<EntregaDestinoTipo>('acolhido');
  ...
  const [distribuicaoForm, setDistribuicaoForm] = useState<DistribuicaoForm>(
    emptyDistribuicaoForm,
  );
  ...
  useEffect(() => {
    ...
    if (acolhidoParam && acolhidos.length > 0) {
      const acolhido = acolhidos.find(
        (item) => item.apiId === Number(acolhidoParam),
      );
      if (acolhido) {
    ...
    if (materialParam && materiais.length > 0) {
      const material = materiais.find(
        (item) => item.id === Number(materialParam),
      );
      if (material) {
  ...
  const totalItens = materiais.reduce(
    (acc, material) => acc + material.estoque_atual,
    0,
  );
  ...
  const baixoEstoque = materiais.filter(
    (material) => material.estoque_atual > 0 && material.estoque_atual <= 5,
  ).length;
  const distribuicoesHoje = entregas.filter(
    (entrega) => entrega.data_entrega === today,
  ).length;

  const categoriasDisponiveis = useMemo(() => {
    return uniqueSorted([
      ...materiais.map((material) => material.categoria),
      ...recebimentos.flatMap((recebimento) =>
        recebimento.itens.map((item) => item.categoria),
      ),
    ]);
  ...
  const materiaisFiltrados = useMemo(() => {
    ...
    return materiais.filter((material) => {
      if (
        query &&
        !`${material.nome} ${material.categoria} ${material.unidade}`
          .toLowerCase()
          .includes(query)
      )
        return false;
      if (categoriaFilter && material.categoria !== categoriaFilter)
        return false;
      if (saldoFilter === 'disponivel' && material.estoque_atual <= 0)
        return false;
      if (
        saldoFilter === 'baixo' &&
        (material.estoque_atual <= 0 || material.estoque_atual > 5)
      )
        return false;
      if (saldoFilter === 'zerado' && material.estoque_atual !== 0)
        return false;
      return true;
    });
  ...
  const cartTotal = readyCartItems.reduce(
  ...
  const setDistribuicaoField = (
    field: keyof DistribuicaoForm,
    value: string,
  ) => {
  ...
  const prefillMaterial = (material: Material) => {
    setCarrinho((current) => {
      if (current.some((item) => item.material?.id === material.id))
        return current;
  ...
  const submitRecebimento = async () => {
    ...
    if (
      !form.nome_abrigo.trim() ||
      !form.municipio_uf ||
      !form.orgao_responsavel ||
      !form.origem.trim() ||
      !form.doador_nome ||
      !form.recebido_por ||
      validItems.length === 0
      ...
      setError(
        'Preencha os dados obrigatórios e selecione pelo menos um material com quantidade.',
      );
      return;
    }
    ...
    try {
      ...
      setError(
        getApiErrorMessage(err, 'Não foi possível salvar o recebimento.'),
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const submitDistribuicao = async () => {
    ...
    if (overStock) {
      setError(
        `Saldo insuficiente para ${overStock.material.nome}. Disponível: ${overStock.material.estoque_atual} ${overStock.material.unidade}.`,
      );
      return;
    }
    ...
    try {
      await createEntregaLote({
        ...
        acolhido_id:
          destinoTipo === 'acolhido' ? (selectedAcolhido?.apiId ?? null) : null,
        familia_id:
          destinoTipo === 'familia' ? (selectedFamilia?.id ?? null) : null,
        externo_nome:
          destinoTipo === 'externo'
            ? distribuicaoForm.externo_nome.trim()
            : null,
        externo_documento:
          destinoTipo === 'externo'
            ? distribuicaoForm.externo_documento.trim() || null
            : null,
        externo_contato:
          destinoTipo === 'externo'
            ? distribuicaoForm.externo_contato.trim() || null
            : null,
        externo_instituicao:
          destinoTipo === 'externo'
            ? distribuicaoForm.externo_instituicao.trim() || null
            : null,
      ...
      setError(
        getApiErrorMessage(err, 'Não foi possível registrar a distribuição.'),
      );
    } finally {
    ...
    <Stack spacing={3}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <MetricCard
            ...
            helper={`${categoriasAtivas} categorias ativas`}
            icon={<CategoryIcon />}
          />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <MetricCard
            title="Baixo estoque"
            value={baixoEstoque}
            helper="Saldo entre 1 e 5"
            icon={<LowPriorityIcon />}
          />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
          <MetricCard
            title="Distribuições hoje"
            value={distribuicoesHoje}
            helper="Linhas de entrega no dia"
            icon={<HistoryIcon />}
          />
        </Grid>
      </Grid>
      ...
      >
        ...
        <Select
          ...
          value={saldoFilter}
          onChange={(event) =>
            setSaldoFilter(event.target.value as SaldoFilter)
          }
          sx={{
            minWidth: 170,
            borderRadius: 999,
                      ...
                      <Chip
                        ...
                        color={
                          material.estoque_atual === 0
                            ? 'default'
                            : material.estoque_atual <= 5
                              ? 'warning'
                              : 'success'
                        }
                        variant={
                          material.estoque_atual === 0 ? 'outlined' : 'filled'
                        }
              ...
              {historicoRecente.map((grupo) => (
                <TableRow key={grupo.id} hover>
                  ...
                  <TableCell>
                    ...
                    <Chip
                      size="small"
                      label={destinoTipoLabel(grupo.destinoTipo)}
                      sx={{ mt: 0.75 }}
                    />
                  </TableCell>
                  <TableCell>
                    <Typography variant="body2">
                      {grupo.itens.length} material(is)
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      {grupo.totalItens} unidade(s)
                    </Typography>
                  </TableCell>
      ...
      <DistribuicaoDrawer
        ...
        onAddCartItem={() =>
          setCarrinho((current) => [...current, { ...emptyCartItem }])
        }
...
function RecebimentoDrawer({
  ...
  return (
    <Drawer
      ...
      open={open}
      onClose={onClose}
      slotProps={{ paper: { sx: { width: { xs: '100vw', md: 860 } } } }}
    >
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    ...
                    onChange={(e) => onField('nome_abrigo', e.target.value)}
                    fullWidth
                    required
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    ...
                    onChange={(e) => onField('municipio_uf', e.target.value)}
                    fullWidth
                    required
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    ...
                    onChange={(e) =>
                      onField('orgao_responsavel', e.target.value)
                    }
                    fullWidth
                    required
                  />
                ...
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <TextField
                    ...
                    onChange={(e) =>
                      onField('data_recebimento', e.target.value)
                    }
                    fullWidth
                    required
                ...
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <TimeInput
                    label="Hora"
                    value={form.hora_recebimento}
                    onChange={(value) => onField('hora_recebimento', value)}
                    required
                  />
                </Grid>
                ...
                <Grid size={{ xs: 12, md: 3 }}>
                  <TextField
                    ...
                    onChange={(e) => onField('origem_outro', e.target.value)}
                    fullWidth
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    label="Nome do doador / instituição"
                    value={form.doador_nome}
                    onChange={(e) => onField('doador_nome', e.target.value)}
                    fullWidth
                    required
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    label="CPF/CNPJ"
                    value={form.doador_documento}
                    onChange={(e) =>
                      onField('doador_documento', e.target.value)
                    }
                    fullWidth
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    ...
                    value={form.doador_contato}
                    onChange={(e) => onField('doador_contato', e.target.value)}
                    fullWidth
                  />
                </Grid>
              </Grid>
                  ...
                  <Grid key={index} container spacing={1.5} alignItems="center">
                    <Grid size={{ xs: 12, md: 4 }}>
                      <Autocomplete
                        ...
                        value={item.material}
                        onChange={(_, value) =>
                          onItem(index, { material: value })
                        }
                        renderInput={(params) => (
                          <TextField
                            {...params}
                            label="Material do catálogo"
                            required
                          />
                        )}
                      />
                    ...
                    <Grid size={{ xs: 8, md: 2 }}>
                      <TextField
                        ...
                        onChange={(e) =>
                          onItem(index, {
                            condicao: e.target.value as 'novo' | 'usado',
                          })
                        }
                        fullWidth
                      >
                    ...
                    <Grid size={{ xs: 12, md: 2.5 }}>
                      <TextField
                        ...
                        onChange={(e) =>
                          onItem(index, { observacoes: e.target.value })
                        }
                        fullWidth
                      />
                    </Grid>
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, md: 3 }}>
                  <TextField
                    ...
                    onChange={(e) => onField('conferido', e.target.value)}
                    fullWidth
                  >
                ...
                <Grid size={{ xs: 12, md: 3 }}>
                  <TextField
                    label="Motivo"
                    value={form.motivo_nao_conferido}
                    onChange={(e) =>
                      onField('motivo_nao_conferido', e.target.value)
                    }
                    fullWidth
                    disabled={form.conferido === 'sim'}
                    required={form.conferido === 'nao'}
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 3 }}>
                  <TextField
                    select
                    label="Restrição ou validade próxima?"
                    value={form.possui_restricao}
                    onChange={(e) =>
                      onField('possui_restricao', e.target.value)
                    }
                    fullWidth
                  >
                    <MenuItem value="nao">Não</MenuItem>
                ...
                <Grid size={{ xs: 12, md: 3 }}>
                  <TextField
                    label="Descrever restrição"
                    value={form.restricao_descricao}
                    onChange={(e) =>
                      onField('restricao_descricao', e.target.value)
                    }
                    fullWidth
                    disabled={form.possui_restricao === 'nao'}
                    required={form.possui_restricao === 'sim'}
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    select
                    label="Destinação inicial"
                    value={form.destinacao_inicial}
                    onChange={(e) =>
                      onField('destinacao_inicial', e.target.value)
                    }
                    fullWidth
                  >
                    ...
                    <MenuItem value="distribuicao_imediata">
                      Distribuição imediata
                    </MenuItem>
                    <MenuItem value="setor_especifico">
                      Encaminhado para setor específico
                    </MenuItem>
                  </TextField>
                </Grid>
                <Grid size={{ xs: 12, md: 8 }}>
                  <TextField
                    label="Local de armazenamento/setor"
                    value={form.local_armazenamento}
                    onChange={(e) =>
                      onField('local_armazenamento', e.target.value)
                    }
                    fullWidth
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    ...
                    onChange={(e) => onField('recebido_por', e.target.value)}
                    fullWidth
                    required
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    label="Função/equipe"
                    value={form.funcao_equipe}
                    onChange={(e) => onField('funcao_equipe', e.target.value)}
                    fullWidth
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    ...
                    onChange={(e) => onField('entregue_por', e.target.value)}
                    fullWidth
                  />
                </Grid>
                <Grid size={{ xs: 12 }}>
                  <TextField
                    ...
                    onChange={(e) =>
                      onField('observacoes_gerais', e.target.value)
                    }
                    multiline
                    minRows={3}
                    fullWidth
...
function DistribuicaoDrawer({
  ...
  return (
    <Drawer
      ...
      open={open}
      onClose={onClose}
      slotProps={{ paper: { sx: { width: { xs: '100vw', md: 820 } } } }}
    >
      <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <DrawerHeader
              ...
              <Stack spacing={2}>
                ...
                {destinoTipo === 'familia' ? (
                  <Autocomplete
                    ...
                    loading={loading}
                    renderInput={(params) => (
                      <TextField {...params} label="Família ativa" required />
                    )}
                  />
                ) : null}
                  ...
                  <Grid container spacing={2}>
                    <Grid size={{ xs: 12, md: 6 }}>
                      <TextField
                        ...
                        value={form.externo_nome}
                        onChange={(e) =>
                          onField('externo_nome', e.target.value)
                        }
                        fullWidth
                        required
                      />
                    ...
                    <Grid size={{ xs: 12, md: 6 }}>
                      <TextField
                        label="Instituição"
                        value={form.externo_instituicao}
                        onChange={(e) =>
                          onField('externo_instituicao', e.target.value)
                        }
                        fullWidth
                      />
                    </Grid>
                    <Grid size={{ xs: 12, md: 6 }}>
                      <TextField
                        label="Documento"
                        value={form.externo_documento}
                        onChange={(e) =>
                          onField('externo_documento', e.target.value)
                        }
                        fullWidth
                      />
                    </Grid>
                    <Grid size={{ xs: 12, md: 6 }}>
                      <TextField
                        label="Contato"
                        value={form.externo_contato}
                        onChange={(e) =>
                          onField('externo_contato', e.target.value)
                        }
                        fullWidth
                      />
                    </Grid>
                  ...
                  <Grid key={index} container spacing={1.5} alignItems="center">
                    <Grid size={{ xs: 12, md: 7 }}>
                      <Autocomplete
                        options={materiais}
                        getOptionLabel={(option) =>
                          `${option.nome} | ${option.categoria} | ${option.estoque_atual} ${option.unidade}`
                        }
                        value={item.material}
                        onChange={(_, value) =>
                          onCartItem(index, {
                            ...
                            quantidade: value
                              ? Math.min(
                                  item.quantidade || 1,
                                  Math.max(value.estoque_atual, 1),
                                )
                              : 1,
                          })
                        }
                    ...
                    <Grid size={{ xs: 12, md: 2 }}>
                      <Typography
                        ...
                        color={
                          item.material &&
                          item.quantidade > item.material.estoque_atual
                            ? 'error.main'
                            : 'text.secondary'
                        }
                      >
                        Saldo:{' '}
                        {item.material
                          ? `${item.material.estoque_atual} ${item.material.unidade}`
                          : 'Selecione'}
                      </Typography>
                    </Grid>
                  </Grid>
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, md: 4 }}>
                  <TextField
                    ...
                    onChange={(e) => onField('finalidade', e.target.value)}
                    fullWidth
                  />
                </Grid>
                <Grid size={{ xs: 12, md: 8 }}>
...
function isReadyRecebimentoItem(
  item: RecebimentoItemForm,
): item is RecebimentoItemForm & { material: Material } {
  return !!item.material && item.quantidade > 0;
}
...
function findOverStockItem(items: ReadyCartItem[]): ReadyCartItem | null {
  ...
  return (
    Array.from(totals.values()).find(
      (item) => item.quantidade > item.material.estoque_atual,
    ) ?? null
  );
}
...
function entregaDestinationLabel(entrega: Entrega): string {
  ...
  if (entrega.destino_tipo === 'externo') {
    return (
      [entrega.externo_nome, entrega.externo_instituicao]
        .filter(Boolean)
        .join(' | ') || 'Destinatário externo'
    );
  }
...
function destinationSummary(
  ...
  if (tipo === 'acolhido')
    return acolhido
      ? `${acolhido.name} | ${acolhido.id}`
      : 'Pessoa acolhida não selecionada';
  if (tipo === 'familia')
    return familia
      ? `${familia.codigo} | ${familia.responsavelNome ?? 'Responsável não informado'}`
      : 'Família não selecionada';
